...
export class AppService {
  getWoman(): any {
  const womanList = women.map(wom => `${wom.name} ${wom.lastName}`);
  return womanList;
...
export class AppService {
  getWoman(): any {
    try {
      // Lee el archivo women.json desde la carpeta 'data'
      const data = fs.readFileSync('./data/women.json', 'utf8');
      const women: iWoman[] = JSON.parse(data);
      
      // Mapea los nombres y apellidos de las mujeres
      const womanList = women.map(wom => `${wom.name} ${wom.lastName}`);
